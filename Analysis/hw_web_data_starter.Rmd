...
```{r}
html_obj <- read_html("Best Picture-Winning (Sorted by Year Descending) - IMDb.htm")
html_obj
```
...
# define MPAA ratings
oscar_df %>% 
  mutate(ismpaa = str_detect(text, "^GP|PG|PG-13|[R]$")) ->
  oscar_df
# oscar_df %>% 
#  filter(ismpaa == T) %>% 
#  nrow()
...
# define genre
oscar_df %>% 
  mutate(isgenre = !ismovierank & !isname & !isyear & !ismpaa & !islength & !isstar & !ismeta & !isgross) ->
  oscar_df
```
